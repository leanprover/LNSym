...
partial def introChangeHyps (goal : MVarId) (hStep : Expr) (hyp_prefix : String):
  TacticM Bool := goal.withContext do
  let step ← inferType hStep
    let_expr Eq _ st_var nest ← step | return false
    ...
    if ! (st_var.isFVar && nest.isApp) then
      throwError m!"[introChangeHyp] Unexpected expression(s). We expect \
      ...
      -- let read_hyp := List.foldl mkAnd (mkConst ``True) reads
      let changed_hyp := List.foldl mkAnd reads.head! reads.tail!
      let unchanged_hyp := unchangedFieldProp st_var prev_st_var seen_fields
...
def introChangeHypsElab (h_step : Name) (hyp_prefix : String): TacticM Unit :=
  withMainContext
  (do
    let h_step ← getFVarFromUserName h_step
    let success ← introChangeHyps (← getMainGoal) h_step hyp_prefix
    if ! success then
      failure)

elab "intro_change_hyps" h_step:ident hyp_prefix:str : tactic =>
  introChangeHypsElab (h_step.getId) (hyp_prefix.getString)
