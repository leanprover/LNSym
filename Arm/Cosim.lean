...
-/
def mk_one_test_task (verbose : Bool) (fn : IO (Option (BitVec 32))) (guid : Nat) :
...
def run_n_tests (verbose : Bool) (n : Nat) (fn : IO (Option (BitVec 32)))
    (tasks : Array (Task (Except IO.Error Bool))) :
    IO (Array (Task (Except IO.Error Bool))) := do
  let mut tasks := tasks
  for _ in [0:n] do
    if let .some t ← mk_one_test_task verbose fn (1 + tasks.size) then
      tasks := tasks.push t
  return tasks
