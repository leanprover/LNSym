...
/--
Make a task for running a single test. Return `some t` if a test can be produced, and `none` if not.
Uses `IO` to try produce a random test that shall be run in a task.
-/
def mk_one_test_task (verbose : Bool) (fn : IO (Option (BitVec 32))) : IO (Option (Task (Except IO.Error Bool))) := do
  let maybe_inst ← fn
...
def run_n_tests (verbose : Bool) (n : Nat) (fn : IO (Option (BitVec 32))) (tasks : Array (Task (Except IO.Error Bool))) :
    IO (Array (Task (Except IO.Error Bool))) := do
  let mut tasks := tasks
  for _ in [0:n] do
    if let .some t ← mk_one_test_task verbose fn then
      tasks := tasks.push t
  return tasks
...
def run_all_tests (verbose : Bool) (n : Nat) : IO UInt32 := do
  ...
  if machine_check.exitCode == 0 then
    ...
    for inst_fn in Insts.rand do
        tasks ← run_n_tests verbose n inst_fn tasks
