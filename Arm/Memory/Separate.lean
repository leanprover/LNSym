...
theorem Memory.write_bytes_read_bytes_eq_self
    (mem : Memory)
    (hlegal : mem_legal' a an) :
    (mem.write_bytes an a (mem.read_bytes an a)) = mem := by
    have hlegal' := hlegal.def
    funext i
    rw [Memory.write_bytes_eq (by bv_omega)]
    rw [extractLsByte_read_bytes (by bv_omega)]
    by_cases h : i < a
    · simp [h]
    · simp only [h, if_false]
      by_cases h₂ : i.toNat ≥ a.toNat + an
      · simp [h₂]
      · simp only [h₂, if_false]
        rw [BitVec.toNat_sub_eq_toNat_sub_toNat_of_le (by bv_omega)]
        simp only [show i.toNat - a.toNat < an by bv_omega, if_true]
        have h : a + BitVec.ofNat 64 (i.toNat - a.toNat) = i := by
          apply BitVec.eq_of_toNat_eq
          bv_omega
        rw [h]
        rfl
