...
def memOmegaTac : MemOmegaM Unit := do
    ...
    if (← readThe Context).cfg.explodePairwiseSeparate then
      let hyps ← getLocalHyps
      let mut foundHyps : Array Memory.Hypothesis := #[]
      for h in hyps do
        foundHyps ← hypothesisOfExpr h foundHyps
        for hyp in foundHyps do
          let Memory.Hypothesis.pairwiseSeparate sep := hyp
            | continue
          -- Add omega facts.
          let _ ← sep.addOmegaFacts #[]


    TacticM.withTraceNode' m!"Reducion to omega" do
      try
        TacticM.traceLargeMsg m!"goal (Note: can be large)"  "{← getMainGoal}"
