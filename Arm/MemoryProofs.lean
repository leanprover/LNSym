  ...
  case neg => -- n ≠ 0
   ...
   case succ =>
     ...
     · have := mem_separate_contiguous_regions addr 0#64 (n - 1)#64
       simp [Std.BitVec.add_zero, Std.BitVec.sub_zero, -Nat.reducePow, -ofNat_lt_ofNat] at this
...
private theorem mem_subset_neq_first_addr_small_second_region
  ...
  case inl =>
    ...
    have l1 : n' = 18446744073709551615 := by
      ...
      simp [Std.BitVec.toNat_ofNat] at h
  ...
  case neg => -- n1 ≠ 0
    ...
    case succ =>
      ...
      have h_sep : mem_separate addr2 addr2 (addr2 + 1#64) (addr2 + 1#64 + (n' - 1)#64) := by
          ...
          simp [Std.BitVec.sub_zero, Std.BitVec.add_zero] at this
          rw [this]
    ...
    · rename_i i _ n1_1 v
      ...
      case neg =>
        ...
        have hn' : n <= n1_1 := by
          ...
          simp [Std.BitVec.toNat_ofNat, -Nat.reducePow] at hn
        ...
        · have rw_lemma2 := @read_mem_of_write_mem_bytes_same_first_address
          ...
          simp [-Nat.reducePow] at rw_lemma2
...
private theorem read_mem_of_write_mem_bytes_subset_helper_1
  ...
  simp [l1, -Nat.reducePow]
  have l2 : a - 1 < 2 ^ 64 := by omega
  simp [Nat.mod_eq_of_lt l2, -Nat.reducePow]
  omega
...
private theorem read_mem_of_write_mem_bytes_subset_helper_4
  (h_v_size : v < 2 ^ ((n' + 1) * 8)) (h_a_base : a ≠ 0) (h_a_size : a < 2 ^ 64) :
  (v >>> 8 % 2 ^ (n' * 8)) >>> ((a + (2 ^ 64 - 1)) % 2 ^ 64 * 8) % 2 ^ 8 = v >>> (a * 8) % 2 ^ 8 := by
  apply Nat.eq_of_testBit_eq; intro i
  simp [Nat.testBit_mod_two_pow, -Nat.reducePow]
  by_cases h₂ : i < 8
  case pos => -- (i < 8)
    simp [h₂, Nat.testBit_shiftRight, Nat.testBit_mod_two_pow, -Nat.reducePow]
...
theorem read_mem_of_write_mem_bytes_subset
  ...
  case succ =>
    rename_i n' n_ih
    simp_all [write_mem_bytes, -Nat.reducePow, -toNat_sub]
    have cast_lemma := @cast_of_extract_eq
    by_cases h₀ : n' = 0
    case pos =>
      subst n'
      simp_all [mem_subset_eq, -Nat.reducePow, -toNat_sub]
      subst addr2
      simp [write_mem_bytes, read_mem_of_write_mem_same, -Nat.reducePow, -toNat_sub]
      have addr1_ub := addr1.isLt
      have : addr1.toNat < 2^64 := by omega
      rw [←cast_lemma] <;> simp [-toNat_sub]
    case neg => -- (n' ≠ 0)
      by_cases h₁ : addr2 = addr1
      case pos => -- (n' ≠ 0) and (addr2 = addr1)
        subst_vars
        rw [read_mem_of_write_mem_bytes_different (by omega)]
        · simp [read_mem_of_write_mem_same, -Nat.reducePow, -toNat_sub]
          have := addr1.isLt
          rw [←cast_lemma] <;> simp
        · rw [mem_separate_contiguous_regions_one_address _ (by omega)]
      case neg => -- (addr2 ≠ addr1)
        rw [n_ih (by omega) (by omega)]
        · ext
          simp only [extractLsb, extractLsb', cast_ofNat, toNat_cast]
          simp only [toNat_zeroExtend, toNat_ofNat, toNat_ofNat]
          simp only [BitVec.sub_of_add_is_sub_sub]
          ...
          have h_sub := toNat_sub (addr2 - addr1) 1#64
          simp only [toNat_ofNat, ne_eq, Nat.succ_ne_zero, not_false_eq_true,
                     Nat.one_lt_pow_iff, Nat.lt_succ_self, Nat.mod_eq_of_lt] at h_sub
          simp only [h_sub]
          rw [read_mem_of_write_mem_bytes_subset_helper_4 h_v_size h_a_base h_a_size]
...
theorem read_mem_bytes_of_write_mem_bytes_subset_helper2
 ...
 (Std.BitVec.toNat val >>> ((Std.BitVec.toNat (addr2 - addr1) + 1) % 2 ^ 64 * 8) %
          2 ^
            (((Std.BitVec.toNat (addr2 - addr1) + 1) % 2 ^ 64 + n) * 8 - 1 -
                (Std.BitVec.toNat (addr2 - addr1) + 1) % 2 ^ 64 * 8 +
              1)) <<<
        8 |||
      Std.BitVec.toNat val >>> (Std.BitVec.toNat (addr2 - addr1) * 8) %
        2 ^ ((Std.BitVec.toNat (addr2 - addr1) + 1) * 8 - 1 - Std.BitVec.toNat (addr2 - addr1) * 8 + 1) =
    Std.BitVec.toNat val >>> (Std.BitVec.toNat (addr2 - addr1) * 8) %
      2 ^ ((Std.BitVec.toNat (addr2 - addr1) + (n + 1)) * 8 - 1 - Std.BitVec.toNat (addr2 - addr1) * 8 + 1) := by
  have h_a_size := (addr2 - addr1).isLt
  have h_v_size := val.isLt
  have h5' : Std.BitVec.toNat (addr2 - addr1) < 2^64 - 1 := by
    rw [←BitVec.toNat_lt (addr2 - addr1) (2^64 - 1)#64] at h5
    simp [-Nat.reducePow, -toNat_sub] at h5
    rw [Nat.mod_eq_of_lt (by decide)] at h5
    assumption
  generalize Std.BitVec.toNat val = v at h_v_size
  generalize Std.BitVec.toNat (addr2 - addr1) = a at h_a_size h5'
  apply Nat.eq_of_testBit_eq; intro i
  simp [Nat.testBit_mod_two_pow, Nat.testBit_shiftRight, Nat.testBit_or, -Nat.reducePow, -toNat_sub]
  have h0 : (((a + 1) % 2 ^ 64 + n) * 8 - 1 - (a + 1) % 2 ^ 64 * 8 + 1) = n * 8 := by
      omega
  have h1 : (a + 1) * 8 - 1 - a * 8 + 1 = 8 := by omega
  have h2 : (a + (n + 1)) * 8 - 1 - a * 8 + 1 = n * 8 + 8 := by omega
  simp [h0, h1, h2, -Nat.reducePow, -toNat_sub]
  by_cases h₀ : (i - 8 < n * 8)
  case pos => -- (i - 8 < n * 8)
    have h₀' : (i < n * 8 + 8) := by omega
    simp [h₀, h₀', -Nat.reducePow, -toNat_sub]
    by_cases h₁ : i < 8
    case pos => -- (i - 8 < n * 8) and (i < 8)
      have h₁' : ¬(8 <= i) := by omega
      simp [h₁, h₁', -Nat.reducePow, -toNat_sub]
    case neg => -- (i - 8 < n * 8) and ¬(i < 8)
      have h₁' : 8 <= i := by omega
      simp [h₁, h₁', -Nat.reducePow, -toNat_sub]
      rw [read_mem_bytes_of_write_mem_bytes_subset_helper1]
      <;> assumption
  case neg => -- ¬(i - 8 < n * 8)
    have h₀' : ¬(i < n * 8 + 8) := by omega
    simp [h₀, h₀', -Nat.reducePow, -toNat_sub]
...
private theorem read_mem_bytes_of_write_mem_bytes_subset_n2_lt
  ...
  case base =>
    simp_all [read_mem_bytes, -Nat.reducePow, -toNat_sub]
  ...
  case succ =>
    ...
    case pos => -- (addr1 = addr2)
      ...
      rw [this]
      ext; simp [-Nat.reducePow, -toNat_sub]
    case neg => -- (addr1 ≠ addr2)
      simp [read_mem_bytes, -Nat.reducePow, -toNat_sub]
      simp [-Nat.reducePow, -toNat_sub] at h4
      have h_sub : mem_subset (addr2 + 1#64) (addr2 + 1#64 + (n - 1)#64) addr1 (addr1 + (n1 - 1)#64) := by
        ...
        · have l0 := @mem_subset_trans (addr2 + 1#64) (addr2 + n#64) addr2 (addr2 + n#64)
                   addr1 (addr1 + (n1 - 1)#64)
          simp [h4, -Nat.reducePow, -toNat_sub] at l0
          rw [first_addresses_add_one_is_subset_of_region_general (by omega) (by omega) (by omega)] at l0
          · simp_all [-Nat.reducePow, -toNat_sub]
          · simp [mem_subset_refl, -Nat.reducePow, -toNat_sub]
        · omega
        · omega
      ...
      simp [l3, -Nat.reducePow, -toNat_sub] at l1
      rw [l1]
      · simp [-Nat.reducePow, -toNat_sub] at h5
        have n_ih' := @n_ih (addr2 + 1#64) addr1 val s (by omega)
        simp [h_sub, -Nat.reducePow, -toNat_sub] at n_ih'
        rw [mem_legal_lemma h2'] at n_ih'
        · simp [-Nat.reducePow, -toNat_sub] at n_ih'
          have h' : (Std.BitVec.toNat (addr2 + 1#64 - addr1) + n) * 8 - 1 - Std.BitVec.toNat (addr2 + 1#64 - addr1) * 8 + 1 = n * 8 := by omega
          have n_ih'' := n_ih' h6 h'
          rw [n_ih'']
          · ext; simp [-Nat.reducePow, -toNat_sub]
            simp [toNat_ofNat, toNat_zeroExtend, BitVec.add_of_sub_sub_of_add,
...
theorem entire_memory_subset_of_only_itself
  ...
  have : 2^64 = 18446744073709551616 := by decide
  unfold my_pow at *
  simp_all [mem_subset, BitVec.add_sub_self_left_64, -Nat.reducePow, -toNat_sub]
  auto
...
theorem entire_memory_subset_legal_regions_eq_addr
  ...
  have : 2^64-1 = 18446744073709551615 := by decide
  unfold my_pow at *
  simp_all [mem_subset, mem_legal, -Nat.reducePow, -toNat_sub]
  auto
  ...
  read_mem_bytes n2 addr2 (write_mem_bytes n1 addr1 val s) =
   Std.BitVec.cast h
    ...
    · apply read_mem_bytes_of_write_mem_bytes_subset_n2_eq_alt_helper
      simp [my_pow_2_gt_zero, -Nat.reducePow, -toNat_sub]
...
theorem leftshift_n_or_rightshift_n (x y : Nat) (h : y < 2^n) :
  (x <<< n ||| y) >>> n = x := by
  apply Nat.eq_of_testBit_eq; intro i
  simp [Nat.testBit_shiftRight, Nat.testBit_or, -Nat.reducePow, -toNat_sub]
  have l0 : y < 2^(n + i) :=
    calc
      _ < 2^n := by assumption
      _ <= 2^(n + i) := by simp [Nat.pow_le_pow_right, -Nat.reducePow, -toNat_sub]
  rw [Nat.testBit_lt_two l0]
  simp [Nat.testBit_shiftLeft, -Nat.reducePow, -toNat_sub]
  done
...
private theorem write_mem_bytes_irrelevant_helper (h : n * 8 + 8 = (n + 1) * 8) :
  ...
  ext
  simp [zeroExtend, -Nat.reducePow, -toNat_sub]
  ...
  simp [h_x, -Nat.reducePow, -toNat_sub] at h_x_size
  simp [h_y, -Nat.reducePow, -toNat_sub] at h_y_size
...
theorem write_mem_bytes_irrelevant :
  ...
  induction n generalizing addr s
  case zero => simp [write_mem_bytes, -Nat.reducePow, -toNat_sub]
  case succ =>
    rename_i n n_ih
    simp [write_mem_bytes, read_mem_bytes, -Nat.reducePow, -toNat_sub]
    conv =>
      pattern write_mem ..
      arg 2
