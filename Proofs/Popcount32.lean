...
/-- Check that symbolic simulation gets the right memory effect -/
theorem popcount32_sym_memory (s0 s_final : ArmState)
    (h_s0_pc : read_pc s0 = 0x4005b4#64)
    (h_s0_program : s0.program = popcount32_program)
    (h_s0_sp_aligned : CheckSPAlignment s0)
    (h_s0_err : read_err s0 = StateError.None)
    (h_run : s_final = run 2 s0)
    (h_s0_x0 : r (.GPR 0#5) s0 = x)
    (h_s0_x31 : r (.GPR 31#5) s0 = y) :
    -- write_mem_bytes 4
    read_mem_bytes 4
      (zeroExtend 64 (zeroExtend (64 + 1) y + 18446744073709551599#65 + 1#65)
      + 12#64)
      s_final
    = zeroExtend 32 x := by
  -- Prelude
  ...
  simp (config := {ground := true}) only [
    h_s1_x31, AddWithCarry,
    h_s1_non_effects (.GPR 0#5) (by decide) (by decide),
    h_s0_x0, h_s0_x31
    ] at h_step_2
  simp [h_step_2, state_simp_rules] -/

  -- However, the memory_effects lemma we got from `AxEffects` does not:
  -- it gives an application type mismatch
  simp [h_s2_memory_effects]
  sorry
  done
