...
/-- Binary representation for
    polynomial <| 1 + x + x^^2 + x^^7 + x^^128 |>
-/
def const : BitVec 129 := 0x100000000000000000000000000000087#129

/-- Binary representation for
    polynomial <| 1 + x^^121 + x^^126 + x^^127 + x^^128 |>
    https://link.springer.com/chapter/10.1007/978-3-031-34671-2_30
-/
def refconst : BitVec 129 := 0x1C2000000000000000000000000000001#129
...
protected def gcm_init_H (H : BitVec 128) : BitVec 128 :=
  pmod (H ++ 0b0#1) refconst (by omega)

protected def gcm_polyval_mul (x : BitVec 128) (y : BitVec 128) : BitVec 256 :=
  0b0#1 ++ pmult x y

protected def gcm_polyval_red (x : BitVec 256) : BitVec 128 :=
  reverse $ pmod (reverse x) const (by omega)

-- TODO: read "A New Interpretation for the GHASH Authenticator of AES-GCM
-- Is it possible to define gcm_polyval like is described in the paper
-- without have to reverse in pmod
